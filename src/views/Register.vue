        ...
        <form @submit.prevent="handleRegister">
          ...
          <div class="hdv-input">
            <input
              ...
              minlength="4"
              maxlength="20"
              v-model="Ruser.password2"
              placeholder="Password Confirmation"
              required
            />
          </div>
          <button class="btn" type="submit">
            <div class="btn__content">Create Free Account</div>
          </button>
          <button class="btn btn_secondary" type="button">
            <a href="/" class="btn__content"> Cancel </a>
          </button>
        </form>
        <div class="card__action">
          <div class="action-text">Already a member?</div>
          <button class="btn btn_secondary" @click="login">
            <a href="/login" class="btn__content"> Sign In </a>
          </button>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    handleRegister() {
      this.$store
        .dispatch('register', this.Ruser)
        .then(() => {
          this.$store.dispatch('login', this.Ruser).then(() => {
